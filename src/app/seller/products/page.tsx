...
// Modals
import { AddProductModal } from '@/components/modals/seller/products/AddProduct';
import { DeleteModal } from '@/components/modals/seller/products/Delete';
import { EditProductModal } from '@/components/modals/seller/products/EditProduct';
...
export default function Products() {
  ...
  const [isShowEditProductModal, setIsShowEditProductModal] = useState<boolean>(false);
  const [products, setProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [mounted, setMounted] = useState<boolean>(false);

  /*** Функций ***/
  const handleToggleAddProductModal = () => setIsShowAddProductModal((prev) => !prev);
  ...
  return (
    <div className={styles['main']}>
      {isShowAddProductModal && <AddProductModal onClose={() => setIsShowAddProductModal(false)} />}
      {isShowDeleteModal && <DeleteModal onClose={() => setIsShowDeleteModal(false)} />}
      {isShowEditProductModal && (
        <EditProductModal onClose={() => setIsShowEditProductModal(false)} />
      )}
      ...
      <div className={styles['header']}>
        <div className={styles['left']}>
          ...
          <button className={styles['header__filter-button']}>
            <FilterIcon />
          </button>
      ...
      <div className={styles['filter']}>
        <div className={cn(styles['filter__nav'], styles['blue'])}>
          <button className={cn(styles['filter__nav-link'], styles['active-blue'])}>
            По остаткам
          </button>
          <button className={styles['filter__nav-link']}>По складам</button>
        </div>

        <div className={cn(styles['filter__nav'], styles['green'])}>
          <button
            onClick={() => setIsArchive(false)}
            className={cn(styles['filter__nav-link'], !isArchive && styles['active-green'])}
          >
            Активные товары
          </button>
      ...
      <div className={styles['table-container']}>
        <table className={styles['table']}>
          ...
          <tbody className={styles['table__body']}>
            {!mounted ? (
              // Show completely empty during SSR to avoid hydration conflicts
              <tr>
              ...
              <tr>
                <td colSpan={7} style={{ textAlign: 'center', padding: '2rem' }}>
                  У вас пока нет товаров. Нажмите "Добавить" чтобы создать первый товар.
                </td>
              </tr>
            ) : (
              products.map((product) => (
                <tr key={product.id} className={styles['table__body-row']}>
                  ...
                  <td className={styles['table__body-td']}>
                    <div className={styles['table__information']}>
                      <div className={styles['table__information-product']}>
                        {product.image ? (
                        ...
                        <p className={styles['table__information-product-name']}>{product.name}</p>
                        <p className={styles['table__information-product-category']}>{product.category}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className={styles['table__body-td']}>
                    <p className={styles['table__text']}>{product.uom}</p>
                  </td>

                  <td className={styles['table__body-td']}>
                    <p className={styles['table__text']}>{product.price.toLocaleString()} ₸</p>
                  </td>
                  ...
                  <td className={styles['table__body-td']}>
                    <div className={cn(styles['table__select'], product.status === 'inactive' && styles['archive'])}>
                      <div></div>
                      <p>{product.status === 'active' ? 'Активный' : 'Неактивный'}</p>
                    </div>
                  </td>
                ...
                <td className={styles['table__body-td']}>
                  <div className={styles['table__body-options']}>
                    ...
                    <button type='button' onClick={() => setIsShowEditProductModal(true)}>
                      <svg
                        xmlns='http://www.w3.org/2000/svg'
                        width='28'
                        height='28'
      ...
      <div className={styles['footer']}>
        <p className={styles['footer__title']}>
          {mounted && !loading ? `Показано ${products.length} товаров` : 'Загрузка...'}
        </p>

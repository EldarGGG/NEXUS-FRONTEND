...
export default function StockRegistration() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.product_id || !formData.storage_id || !formData.amount) {
    ...
    try {
      ...
      if (response.ok) {
        alert('Оприходование успешно создано!');
      ...
      alert('Ошибка создания оприходования');
    }
  };
    ...
    <div className={styles['main']}>
      ...
      {showAddModal && (
        <div className={styles['modal']}>
          ...
          <div className={styles['modal__main']}>
            <h1 className={styles['title']}>Оприходование товара</h1>
        ...
        <table className={styles['table']}>
          <thead className={styles['table__head']}>
            <tr className={styles['table__head-row']}>
              ...
              <th className={styles['table__head-th']}>Поставщик</th>
          ...
          <tbody className={styles['table__body']}>
            {!mounted ? (
              ...
              <tr>
                <td colSpan={8} style={{ textAlign: 'center', padding: '2rem' }}>
                  Нет записей об оприходовании. Нажмите "Добавить" чтобы создать первую запись.
                </td>
              </tr>
            ) : (
              registrations.map((registration) => (
                <tr key={registration.id} className={styles['table__body-row']}>
                  ...
                  <td className={styles['table__body-td']}>
                    <p className={styles['table__text']}>{registration.amount}</p>
                  </td>
                  <td className={styles['table__body-td']}>
      ...
      <div className={styles['footer']}>
        <p className={styles['footer__title']}>
          Показано {registrations.length} записей об оприходовании
        </p>
      </div>
    </div>
  );

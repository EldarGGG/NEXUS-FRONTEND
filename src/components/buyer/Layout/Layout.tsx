...
export function Layout({ children, isGeneralCatalog = false }: LayoutProps) {
  ...
  function setActive(str: string) {
    if (typeof window !== 'undefined') {
      localStorage.setItem('activeSideCategory', str);
    }
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      setActiveCategory(localStorage.getItem('activeSideCategory') || '');
    }
    getCartSize();
  }, [localCart]);
  ...
  return (
    <div className={styles['layout']}>
      <aside className={styles['sidebar']}>
        ...
        <nav className={cn(styles['nav'], navIsOpen && styles['active'])}>
          <ul className={cn(styles['nav__list'], navIsOpen && styles['active'])} >
            <li className={styles['nav__item']}>
              <Link
                href={`/shop/${magazine}`}
                onClick={() => setActive('')}
                className={cn(styles['nav__link'], '' === activeCategory ? styles['active'] : '')}
              >
                <LogoIcon />
            ...
            <li className={cn(styles['nav__item'], navIsOpen && styles['active'])}>
              <Link
                href='#'
                onClick={() => setActiveCategory('favorites')}
                className={cn(
                  styles['nav__link'],
                  'favorites' === activeCategory ? styles['active'] : '',
                )}
              >
                <SavedIcon />
                <span>Избранные</span>
            ...
            <li className={cn(styles['nav__item'], navIsOpen && styles['active'])}>
              <Link
                href={`/shop/${magazine}/orders`}
                onClick={() => setActive('orders')}
                className={cn(
                  styles['nav__link'],
                  'orders' === activeCategory ? styles['active'] : '',
                )}
              >
                <OrdersIcon />
                <span>Заказы</span>
            ...
            <li className={cn(styles['nav__item'], navIsOpen && styles['active'])}>
              <Link
                href='#'
                onClick={() => setActive('caht')}
                className={cn(
                  styles['nav__link'],
                  'chat' === activeCategory ? styles['active'] : '',
                )}
              >
                <ChatIcon />
                <span>Чат</span>
            ...
            <li className={cn(styles['nav__item'], navIsOpen && styles['active'])}>
              <Link
                ...
                className={cn(
                  styles['nav__link'],
                  'settings' === activeCategory ? styles['active'] : '',
                )}
              >
                <SettingsIcon />
                <span>Настройки</span>
            ...
            <li className={cn(styles['nav__item'], navIsOpen && styles['active'])}>
              <Link
                ...
                className={cn(
                  styles['nav__link'],
                  'info' === activeCategory ? styles['active'] : '',
                )}
              >
                <InfoIcon />
            ...
            <li className={cn(styles['nav__item'], navIsOpen && styles['active'])} onClick={toggleNav}>
              <span>Показать все </span> <span>❯</span>
            </li>
          </ul>
        </nav>
      ...
      <div className={styles['wrapper']}>
        ...
        {isMenuOpen && <Menu isGeneralCatalog={isGeneralCatalog} />}
        {isCartOpen && (
          <CartModal
            onClose={() => {
              setIsCartOpen(false);
